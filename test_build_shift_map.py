...
from after5 import CommitTimeInfo, build_shift_map
...
def test_all_after_work_end():
    sm = build_shift_map([make("a", 18 * 3600), make("b", 19 * 3600)], skip_weekends=True, work_end=WORK_END)
    assert sm == {}


def test_all_before_work_end_uniform_shift():
    sm = build_shift_map([make("a", 9 * 3600), make("b", 10 * 3600)], skip_weekends=True, work_end=WORK_END)
...
def test_single_before_work_end():
    sm = build_shift_map([make("a", 14 * 3600)], skip_weekends=True, work_end=WORK_END)
...
def test_after_work_end_untouched():
    ...
    sm = build_shift_map([make("a", 15 * 3600), make("b", 20 * 3600)], skip_weekends=True, work_end=WORK_END)
    assert sm["a"] == _day_offset(WEEKDAY) + WORK_END
    assert "b" not in sm
...
def test_work_start_skips_early_morning():
    ...
    WORK_START = 9 * 3600
    sm = build_shift_map([make("a", 7 * 3600)], skip_weekends=True, work_end=WORK_END, work_start=WORK_START)
    assert sm == {}


def test_work_start_shifts_workday_commit():
    WORK_START = 9 * 3600
    sm = build_shift_map([make("a", 10 * 3600)], skip_weekends=True, work_end=WORK_END, work_start=WORK_START)
    assert sm["a"] == _day_offset(WEEKDAY) + WORK_END
...
def test_weekend_skip():
    sm = build_shift_map([make("a", 9 * 3600, day=WEEKEND)], skip_weekends=True, work_end=WORK_END)
    assert sm == {}


def test_weekend_include():
    sm = build_shift_map([make("a", 9 * 3600, day=WEEKEND)], skip_weekends=False, work_end=WORK_END)
...
def test_mixed_timezones_same_local_day():
    ...
    sm = build_shift_map([c1, c2], skip_weekends=True, work_end=WORK_END)
